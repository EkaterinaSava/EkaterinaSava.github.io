...
Ссылки на пулл-реквест в этот раз нет: я перестала понимать, что происходит в проекте, попыталась скомпиллировать д/з из работ других учеников, но окончательно запуталась и решила, что так не годится делать домашку — надо пересматривать лекции и как-то разбираться.
...
+ Добавили в проект простой сервер, который умеет отдавать данные (чтобы мы мошли делать к нему запрос) плюс добавили в дев-зависимости npm-пакет `concurrently`, чтобы можно было одновременно запускать прокси-сервер с api и обычные npm-скрипты для разработки проекта
